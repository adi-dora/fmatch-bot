...
class RoleMenuSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        menu_roles = [interaction.guild.get_role(val['role']) for val in self.menu['roles']]
        ...
        await interaction.response.send_message("You have been given the selected roles successfully!", ephemeral=True)
...
class RoleMenu(commands.Cog):
    ...
    async def on_ready(self):
        ...
        for role_menu in menu['menus']:
            self.bot.add_view(RoleMenuView(role_menu), message_id=role_menu['message_id'])
    ...
    async def create(self, interaction: discord.Interaction, menu_id: int | None):
        ...
        for menu in menus:
            ...
            msg = menu['message']
            file = menu['file_path'] if menu['file_path'] is not None else discord.utils.MISSING
