...
class MatchView(discord.ui.View):
    ...
    async def enterQueue(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class Match(commands.Cog):
    ...
    match = app_commands.Group(name='match', description= "Create and manage the matchmaking functionality")
    ...
    async def message(self, interaction: discord.Interaction, msg: str | None):
        ...
        if msg is None:
            if match['message'] is None:
                return await interaction.response.send_message('There is no message set!')
            
            return await interaction.response.send_message('``The current message is set to:\n``' + match['message'])
    ...
    async def channel(self, interaction: discord.Interaction, channel: discord.TextChannel | None):
        ...
        if channel is None:
            if match['channel'] is None:
                return await interaction.response.send_message('The channel has not been set!')
            
            return await interaction.response.send_message(f'The current channel is set to: {interaction.guild.get_channel(match["channel"]).mention}')
        ...
        await interaction.response.send_message(f'The channel has been set to: {channel.mention}')
    ...
    async def link(self, interaction: discord.Interaction, url: str |None):
        ...
        if url is None:
            if match['link'] is None:
                return await interaction.response.send_message('There is no image/link stored')
            
            return await interaction.response.send_message('The current image is: ' + match['link'])
