...
def create_profile_embed(user: discord.Member) -> tuple:
    ...
    if (
        female_verified in user.roles
        or male_verified in user.roles
        or m_trans_verified in user.roles
        or f_trans_verified in user.roles
    ):
        g_verified = True
...
def get_selfie(user: discord.Member):
    return discord.File(
        profile_json["profiles"][str(user.id)]["selfie"], filename="img.png"
    )
